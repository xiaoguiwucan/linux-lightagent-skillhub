...
- Skill Hub schema 和校验器同步限制触发词数量、技能内相对文件路径与 50000 字符硬上限；预加载仍需通过 stable room/member ACL，失败时由客户端回退普通技能读取。

验证记录：`.venv/bin/python scripts/validate.py` 校验 6 个技能通过，`.venv/bin/python -m unittest discover -s tests -v` 共 40 项通过，Registry 索引成功重建且 `git diff --check` 通过。主项目通过与本版本相同边界的 `1.0.0` 兼容声明完成两次本地真实链路测试：每次一次预载 6 个文件共 9333 字符，第一轮直接生图，无重复读取、无后续文字回合；两张 1280x720 图片均发送到原群并取得数据库 `confirmed`，端到端耗时分别为 44.880 秒和 36.402 秒。
